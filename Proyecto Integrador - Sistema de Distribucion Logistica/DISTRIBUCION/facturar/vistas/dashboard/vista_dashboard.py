...
5 SOLAPAS: Métricas, Preventistas, Productos, Pedidos Pendientes (con parpadeo), Pedidos Armados.
"""
...
class VistaDashboard(QWidget):
    def __init__(self, db: sqlite3.Connection, parent=None):
        ...
        self.timer_parpadeo.setInterval(2000)  # 2 segundos
    ...
    def _crear_tab_pedidos_armados(self):
        ...
        self.tabla_armados.setColumnCount(6)
        self.tabla_armados.setHorizontalHeaderLabels(["Factura", "Cliente", "Fecha", "Total", "Procesado", "Observaciones"])
        
        self.tabla_armados.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Interactive)
        self.tabla_armados.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)
        self.tabla_armados.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.Interactive)
        self.tabla_armados.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.Interactive)
        self.tabla_armados.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeMode.Interactive)
        self.tabla_armados.horizontalHeader().setSectionResizeMode(5, QHeaderView.ResizeMode.Interactive)
        
        self.tabla_armados.setColumnWidth(0, 120)
        self.tabla_armados.setColumnWidth(2, 100)
        self.tabla_armados.setColumnWidth(3, 120)
        self.tabla_armados.setColumnWidth(4, 150)
    ...
    def cargar_armados(self):
        ...
        for i, p in enumerate(historial):
            self.tabla_armados.setItem(i, 0, QTableWidgetItem(p['numero_factura']))
            self.tabla_armados.setItem(i, 1, QTableWidgetItem(p['cliente_nombre']))
            self.tabla_armados.setItem(i, 2, QTableWidgetItem(p['fecha']))
            
            item_total = QTableWidgetItem(f"${p['total']:,.2f}")
            item_total.setTextAlignment(Qt.AlignmentFlag.AlignRight)
            self.tabla_armados.setItem(i, 3, item_total)
            
            fecha_procesado = p['fecha_procesado'][:16] if p['fecha_procesado'] else ''
            self.tabla_armados.setItem(i, 4, QTableWidgetItem(fecha_procesado))
            self.tabla_armados.setItem(i, 5, QTableWidgetItem(p.get('observaciones', '') or '-'))
            
            # Guardar factura_id
            self.tabla_armados.item(i, 0).setData(Qt.ItemDataRole.UserRole, p['factura_id'])

    def _toggle_parpadeo_tab(self):
        """
        Alterna el color de la solapa cada 2 segundos.
        Rojo fuerte cuando está activo, blanco cuando no.
        """
        try:
            pedidos_pendientes = self.controlador_ventas.contar_pedidos_pendientes()
    ...
    def procesar_pedido(self):
        ...
        try:
            ...
            # ✅ REFRESCAR LA LISTA DE PEDIDOS INMEDIATAMENTE
            self.cargar_pedidos()
            self.cargar_armados()
            
            # ✅ ACTUALIZAR EL TÍTULO DE LA SOLAPA CON PARPADEO
            self.iniciar_parpadeo_pedidos()
            
            # ✅ FORZAR REFRESCO VISUAL
            self.tabla_pedidos.repaint()
            self.tabla_pedidos.viewport().update()
            
            print(f"✅ Pedido {numero_factura} procesado y listas actualizadas")
    ...
    def ver_detalle_armado(self):
        ...
        numero_factura = self.tabla_armados.item(fila, 0).text()
        cliente_nombre = self.tabla_armados.item(fila, 1).text()
        total = self.tabla_armados.item(fila, 3).text()
        fecha = self.tabla_armados.item(fila, 2).text()
        fecha_procesado = self.tabla_armados.item(fila, 4).text()
        observaciones = self.tabla_armados.item(fila, 5).text()
    ...
    def _mostrar_detalle_armado(self, detalles, numero_factura, cliente_nombre, fecha, total, fecha_procesado, observaciones):
        ...
        datos.addWidget(QLabel("<b>Armado el:</b>"), 1, 2)
        datos.addWidget(QLabel(fecha_procesado), 1, 3)
        
        if observaciones and observaciones != '-':
            datos.addWidget(QLabel("<b>Observaciones:</b>"), 2, 0)
            datos.addWidget(QLabel(observaciones), 2, 1, 1, 3)
    ...
    def verificar_notas_pendientes(self):
        ...
        try:
            ...
            nota_modelo = NotaVenta(self.db)
            pendientes = nota_modelo.listar_por_estado('PENDIENTE')
            ...
            print(f"📊 Notas pendientes en dashboard: {cantidad}")
            return cantidad
        except Exception as e:
