...
==================================================
Vista de Facturación con UUID
==================================================
📌 USO: Emisión de facturas y gestión de historial
📌 CARACTERÍSTICAS:
    - Emisión de facturas con UUID
    - Historial de facturas
    - Impresión de PDF con QR
...
class VistaFacturacion(QDialog):
    ...
    def _crear_tab_facturacion(self):
        ...
        self.cmb_tipo.addItems(["A", "B", "C"])
        form_enc.addWidget(self.cmb_tipo, 1, 1)
        
        form_enc.addWidget(LabelCampoAzul("Número Factura"), 1, 2)
        self.txt_numero = LineEditBlanco()
        self.txt_numero.setText(self._generar_numero_factura())
        self.txt_numero.setReadOnly(True)
        form_enc.addWidget(self.txt_numero, 1, 3)
        ...
        self.tabla_detalle.setHorizontalHeaderLabels(["ID", "Producto", "Cantidad", "Precio Unit.", "Subtotal"])
        self.tabla_detalle.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)
        self.tabla_detalle.setColumnWidth(0, 100)
        ...
        for btn in [self.btn_emitir, self.btn_limpiar, self.btn_cancelar, self.btn_sincronizar]:
            btn.setMinimumHeight(32)

        self.btn_emitir.clicked.connect(self.emitir_factura)
        self.btn_limpiar.clicked.connect(self.limpiar_todo)
        self.btn_cancelar.clicked.connect(self.reject)
        self.btn_sincronizar.clicked.connect(self.sincronizar_facturas)

        botones_layout.addStretch()
        botones_layout.addWidget(self.btn_emitir)
        botones_layout.addWidget(self.btn_limpiar)
        botones_layout.addWidget(self.btn_sincronizar)
    ...
    def _crear_tab_historial(self):
        ...
        self.tabla_historial.setHorizontalHeaderLabels(["ID", "N° Factura", "Fecha", "Cliente", "Tipo", "Total", "Estado"])
        ...
        self.tabla_historial.horizontalHeader().setSectionResizeMode(6, QHeaderView.ResizeMode.Interactive)
        
        self.tabla_historial.setColumnWidth(0, 100)
        self.tabla_historial.setColumnWidth(1, 120)
        self.tabla_historial.setColumnWidth(2, 100)
        self.tabla_historial.setColumnWidth(4, 60)
    ...
    def _actualizar_numero_factura(self):
        self.ultimo_numero += 1
        cur = self.db.cursor()
        cur.execute("UPDATE parametros SET ultimo_numero_factura = ? WHERE id = 1", (self.ultimo_numero,))
        self.db.commit()
        self.txt_numero.setText(self._generar_numero_factura())
    ...
    def _cargar_productos(self):
        ...
        for prod in productos[:100]:
            nombre = f"{prod['codigo_producto'][:10]} - {prod['descripcion'][:30]}"
    ...
    def _confirmar(self, titulo, texto):
        msg = QMessageBox(self)
        msg.setWindowTitle(titulo)
        msg.setText(texto)
    ...
    def agregar_producto(self):
        ...
        self.detalle_temporal.append((producto_id, cantidad, precio, prod['codigo_producto'], prod['descripcion']))
    ...
    def _refrescar_tabla(self):
        ...
        for fila, (producto_id, cant, precio, codigo, desc) in enumerate(self.detalle_temporal):
            subtotal = cant * precio
            self.tabla_detalle.setItem(fila, 0, QTableWidgetItem(producto_id[:8] + "..."))
            self.tabla_detalle.setItem(fila, 1, QTableWidgetItem(desc[:35]))
            item_cant = QTableWidgetItem(str(cant))
            item_cant.setTextAlignment(Qt.AlignmentFlag.AlignRight)
            self.tabla_detalle.setItem(fila, 2, item_cant)
            item_precio = QTableWidgetItem(f"${precio:,.2f}")
            item_precio.setTextAlignment(Qt.AlignmentFlag.AlignRight)
            self.tabla_detalle.setItem(fila, 3, item_precio)
            item_subtotal = QTableWidgetItem(f"${subtotal:,.2f}")
            item_subtotal.setTextAlignment(Qt.AlignmentFlag.AlignRight)
            self.tabla_detalle.setItem(fila, 4, item_subtotal)
            
            self.tabla_detalle.item(fila, 0).setData(Qt.ItemDataRole.UserRole, producto_id)

    def _calcular_totales(self):
        total_general = 0.0
        for _, cant, precio, _, _ in self.detalle_temporal:
    ...
    def limpiar_todo(self):
        if self.detalle_temporal:
            confirm = self._confirmar("Limpiar", "¿Eliminar todos los productos agregados?")
    ...
    def emitir_factura(self):
        ...
        try:
            items = []
            for producto_id, cant, precio, codigo, desc in self.detalle_temporal:
                items.append({
            ...
            factura_id = self.controlador.emitir_factura_directa(
    ...
    def cargar_historial(self):
        ...
        for i, f in enumerate(facturas):
            self.tabla_historial.setItem(i, 0, QTableWidgetItem(f['id'][:8] + "..."))
            self.tabla_historial.setItem(i, 1, QTableWidgetItem(f['numero_factura']))
            self.tabla_historial.setItem(i, 2, QTableWidgetItem(f['fecha']))
            self.tabla_historial.setItem(i, 3, QTableWidgetItem(f['cliente']))
            self.tabla_historial.setItem(i, 4, QTableWidgetItem(f['tipo_comprobante']))
            
            item_total = QTableWidgetItem(f"${f['total']:,.2f}")
            item_total.setTextAlignment(Qt.AlignmentFlag.AlignRight)
            self.tabla_historial.setItem(i, 5, item_total)
    ...
    def ver_detalle_factura_impresa(self):
        ...
        numero_factura = self.tabla_historial.item(fila, 1).text()
    ...
    def _mostrar_factura_impresa(self, factura_id, numero_factura):
        ...
        tabla_detalle.setHorizontalHeaderLabels(["Código", "Producto", "Cantidad", "Precio Unit.", "Subtotal"])
        ...
        for i, det in enumerate(detalles):
            ...
            item_subtotal.setTextAlignment(Qt.AlignmentFlag.AlignRight)
            tabla_detalle.setItem(i, 4, item_subtotal)
            total += det['subtotal']
    ...
    def anular_factura(self):
        ...
        factura_id = self.tabla_historial.item(fila, 0).data(Qt.ItemDataRole.UserRole)
        numero_factura = self.tabla_historial.item(fila, 1).text()
        estado = self.tabla_historial.item(fila, 6).text()
        ...
        try:
            self.controlador.anular_factura(factura_id, "Anulación manual desde sistema")
    ...
    def sincronizar_facturas(self):
        try:
            resultado = sincronizar_ahora(self.db)
            self.cargar_historial()
            self._mostrar_mensaje("Sincronización", "✅ Facturas sincronizadas con Turso")
        except Exception as e:
    ...
    def _generar_pdf_factura(self, cliente_id, numero, tipo, observaciones):
        try:
            ...
            for producto_id, cant, precio, codigo, desc in self.detalle_temporal:
                ...
                data.append([codigo[:10], desc[:40], f"{cant:.1f}", f"${precio:,.2f}", f"${subtotal:,.2f}"])
            
            tabla = Table(data, colWidths=[60, 170, 50, 70, 70])
            ...
            def add_watermark(canvas_obj, doc_obj):
                ...
                canvas_obj.drawCentredString(0, 0, "SOLO A MODO DE PRUEBA")
                canvas_obj.setFont('Helvetica-Bold', 22)
                canvas_obj.drawCentredString(0, -35, "LA FACTURA CARECE DE VALIDEZ")
