...
==================================================
Vista de Cuenta Corriente con UUID
==================================================
📌 USO: Gestión de cuenta corriente de clientes
📌 CARACTERÍSTICAS:
    - Saldos y movimientos con UUID
    - Registro de cobros
    - Lista de deudores
...
from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QGridLayout, QLabel,
                               ...
                               QWidget, QFileDialog, QScrollArea, QTextEdit)
...
class VistaCuentaCorriente(QDialog):
    def __init__(self, db: sqlite3.Connection, parent=None):
        ...
        self.lbl_id.setStyleSheet("font-size: 10px; color: #666666;")
        ...
        resumen_layout.addWidget(self.lbl_id)
    ...
    def _crear_tab_movimientos(self):
        ...
        self.tabla_movimientos.setHorizontalHeaderLabels(
            ["ID", "Fecha", "Tipo", "Referencia", "Importe", "Saldo", "Observaciones"]
        )
        self.tabla_movimientos.horizontalHeader().setSectionResizeMode(6, QHeaderView.ResizeMode.Stretch)
        self.tabla_movimientos.setColumnWidth(0, 100)
    ...
    def _crear_tab_cobros(self):
        ...
        self.tabla_facturas_pendientes.setColumnCount(5)
        self.tabla_facturas_pendientes.setHorizontalHeaderLabels(
            ["ID", "Factura", "Fecha", "Total", "Pendiente"]
        )
        self.tabla_facturas_pendientes.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)
        self.tabla_facturas_pendientes.setColumnWidth(0, 100)
        ...
        self.btn_sincronizar.clicked.connect(self.sincronizar_cobros)
    ...
    def _crear_tab_deudores(self):
        ...
        self.tabla_deudores.setColumnCount(4)
        self.tabla_deudores.setHorizontalHeaderLabels(
            ["ID", "Razón Social", "Teléfono", "Saldo"]
        )
        self.tabla_deudores.setColumnWidth(0, 100)
        self.tabla_deudores.setColumnWidth(1, 380)
        self.tabla_deudores.setColumnWidth(2, 120)
        self.tabla_deudores.setColumnWidth(3, 120)
        ...
        btn_sincronizar.setStyleSheet("background-color: #4CAF50; color: white; border-radius: 4px; padding: 6px 12px; font-weight: bold;")
        btn_sincronizar.clicked.connect(self.sincronizar_deudores)
    ...
    def cargar_resumen_cliente(self):
        ...
        if cliente:
            ...
            self.lbl_id.setText(f"🆔 ID: {cliente['id'][:8]}...")
    ...
    def cargar_movimientos(self):
        ...
        for i, mov in enumerate(movimientos):
            self.tabla_movimientos.setItem(i, 0, QTableWidgetItem(mov['id'][:8] + "..."))
            self.tabla_movimientos.setItem(i, 1, QTableWidgetItem(mov['fecha']))
            ...
            self.tabla_movimientos.setItem(i, 2, item_tipo)
            
            ref = mov['referencia_id']
            self.tabla_movimientos.setItem(i, 3, QTableWidgetItem(ref[:12] + "..." if len(ref) > 12 else ref))
            ...
            self.tabla_movimientos.setItem(i, 4, item_importe)
            
            self.tabla_movimientos.setItem(i, 5, QTableWidgetItem(f"${mov['saldo_resultante']:,.2f}"))
            self.tabla_movimientos.setItem(i, 6, QTableWidgetItem(mov.get('observaciones', '')))
    ...
    def cargar_facturas_pendientes(self):
        ...
        for i, fact in enumerate(facturas):
            self.tabla_facturas_pendientes.setItem(i, 0, QTableWidgetItem(fact['id'][:8] + "..."))
            self.tabla_facturas_pendientes.setItem(i, 1, QTableWidgetItem(fact['numero_factura']))
            self.tabla_facturas_pendientes.setItem(i, 2, QTableWidgetItem(fact['fecha']))
            self.tabla_facturas_pendientes.setItem(i, 3, QTableWidgetItem(f"${fact['total']:,.2f}"))
            self.tabla_facturas_pendientes.setItem(i, 4, QTableWidgetItem(f"${fact['saldo_pendiente']:,.2f}"))
    ...
    def cargar_tabla_deudores(self):
        ...
        for i, cli in enumerate(clientes):
            self.tabla_deudores.setItem(i, 0, QTableWidgetItem(cli['id'][:8] + "..."))
            self.tabla_deudores.setItem(i, 1, QTableWidgetItem(cli['razon_social']))
            self.tabla_deudores.setItem(i, 2, QTableWidgetItem(cli['telefono'] or "-"))
            ...
            self.tabla_deudores.setItem(i, 3, item_saldo)
    ...
    def exportar_deudores_csv(self):
        ...
        if filename:
            ...
            with open(filename, 'w', newline='', encoding='utf-8-sig') as f:
                writer = csv.writer(f)
                writer.writerow(["ID", "Razón Social", "Teléfono", "Saldo"])
                for cli in clientes:
                    writer.writerow([
                        cli['id'][:8],
                        cli['razon_social'],
    ...
    def registrar_cobro(self):
        ...
        try:
            cobro_id = self.controlador.registrar_cobro(
    ...
    def sincronizar_cobros(self):
        try:
            resultado = sincronizar_ahora(self.db)
            self.cargar_datos_cliente()
            self.cargar_tabla_deudores()
            self._mostrar_mensaje("Sincronización", "✅ Cobros sincronizados con Turso")
        except Exception as e:
            self._mostrar_mensaje("Error", f"Error en sincronización: {e}", QMessageBox.Icon.Critical)

    def sincronizar_deudores(self):
        try:
            resultado = sincronizar_ahora(self.db)
            self.cargar_tabla_deudores()
            self._mostrar_mensaje("Sincronización", "✅ Deudores sincronizados con Turso")
        except Exception as e:
            self._mostrar_mensaje("Error", f"Error en sincronización: {e}", QMessageBox.Icon.Critical)
    ...
    def exportar_movimientos(self):
        ...
        if filename:
            ...
            with open(filename, 'w', newline='', encoding='utf-8-sig') as f:
                writer = csv.writer(f)
                writer.writerow(["ID", "Fecha", "Tipo", "Referencia", "Importe", "Saldo", "Observaciones"])
