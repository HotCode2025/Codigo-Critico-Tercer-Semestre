...
==================================================
PARTE 8.1: Ventana Principal con UUID
==================================================
📌 USO: Ventana principal del sistema
📌 CARACTERÍSTICAS:
    - Panel lateral con botones circulares
    - Barra de estado con notificaciones
    - Sincronización automática con Turso (sin splash duplicado)
...
from vistas.sincronizacion.vista_monitor import abrir_monitor_sincronizacion
...
from utilidades.notificador_sync import registrar_actualizador
...
class VentanaPrincipal(QMainWindow):
    def __init__(self, usuario=None, gif_acerca=None):
        super().__init__()
        print("🚀 Iniciando VentanaPrincipal con UUID...")
        ...
        accion_monitor = QAction("📡 Monitor de Sincronización", self)
        accion_monitor.triggered.connect(self.abrir_monitor_sincronizacion)
        menu_herramientas.addAction(accion_monitor)
        ...
        # ✅ Sincronizar Ahora - Usa el splash real de main.py
        accion_sincronizar = QAction("🔄 Sincronizar Ahora", self)
        accion_sincronizar.triggered.connect(self.sincronizar_ahora)
        menu_herramientas.addAction(accion_sincronizar)
        ...
        if verificar_conexion_turso():
            self.status_bar.showMessage("✅ Conectado a Turso")
        else:
            self.status_bar.showMessage("⚠️ Sin conexión a Turso")
        ...
        # ========== INICIAR SINCRONIZACIÓN AUTOMÁTICA (SIN SPLASH, YA SE HIZO EN MAIN) ==========
        self._iniciar_sincronizacion_auto()

        print("✅ VentanaPrincipal inicializada correctamente con UUID")
        print(f"📐 Escala activa: {os.environ.get('QT_SCALE_FACTOR', '1.0')}")

    def _iniciar_sincronizacion_auto(self):
        """Inicia la sincronización automática en segundo plano (sin splash, ya se hizo en main)"""
        try:
            self.sincronizador = iniciar_sincronizacion_auto(self.db, intervalo=5)
            self.sincronizador.agregar_listener(self._on_sync_event)
    ...
    def _on_sync_event(self, evento, datos):
        ...
        if evento == 'sincronizado':
            ...
            self.status_bar.showMessage(f"🔄 Sincronización completada: {hora_actual}")
            self.verificar_notas_pendientes()
        elif evento == 'error':
            error_msg = datos.get('error', 'Error desconocido')
            self.status_bar.showMessage(f"⚠️ Error en sincronización: {error_msg}")
        elif evento == 'iniciado':
            self.status_bar.showMessage("🔄 Sincronización automática iniciada")
        elif evento == 'detenido':
            self.status_bar.showMessage("⏹️ Sincronización automática detenida")
    ...
    def actualizar_barra_estado(self):
        try:
            ...
            if cantidad > 0:
                ...
                if verificar_conexion_turso():
                    self.status_bar.showMessage("✅ Sistema operativo - Conectado a Turso")
                else:
                    self.status_bar.showMessage("⚠️ Sistema operativo - Sin conexión a Turso")
    ...
    def sincronizar_ahora(self):
        """Ejecuta sincronización manual - usa el splash real de main.py"""
        try:
            # Importar el splash real desde main
            from main import SplashSincronizacionReal
            
            # Mostrar splash y sincronizar
            splash = SplashSincronizacionReal(self.db)
            splash.show()
            QApplication.processEvents()
            
            exito = splash.ejecutar_y_esperar(timeout=300)
            splash.close()
            
            if exito:
                self.status_bar.showMessage("🔄 Sincronización manual completada")
                self.verificar_notas_pendientes()
                QMessageBox.information(self, "Sincronización", "✅ Sincronización completada correctamente.")
            else:
                self.status_bar.showMessage("⚠️ Error en sincronización manual")
                QMessageBox.warning(self, "Sincronización", "⚠️ La sincronización no se completó correctamente.")
        except Exception as e:
    ...
    def abrir_monitor_sincronizacion(self):
        try:
            from vistas.sincronizacion.vista_monitor import abrir_monitor_sincronizacion
            abrir_monitor_sincronizacion(self.db, self)
        except Exception as e:
    ...
    def abrir_notas_venta(self):
        try:
            dialogo = VistaNotasVenta(self, self.db)
            # Registrar la vista para actualización automática
            registrar_actualizador(dialogo.refrescar_desde_sincronizacion)
            dialogo.exec()
            self.verificar_notas_pendientes()
        except Exception as e:
