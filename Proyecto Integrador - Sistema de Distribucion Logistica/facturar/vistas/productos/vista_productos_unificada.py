...
==================================================
Vista Unificada de Productos, Stock y Catálogo con UUID
==================================================
📌 USO: Gestión de productos, stock y catálogo
📌 CARACTERÍSTICAS:
    - CRUD completo con UUID
    - Gestión de lotes
    - Catálogo visual con imágenes desde carpeta local
"""
...
from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QGridLayout, QLabel,
                               ...
                               QTabWidget, QScrollArea, QInputDialog, QDateEdit)
...
class TarjetaProducto(QFrame):
    """Tarjeta de producto para el catálogo con soporte para imágenes locales"""
    ...
    def __init__(self, producto, parent=None):
        ...
        # ============================================================
        # ✅ IMAGEN CORREGIDA - Soporta url_foto, BLOB y búsqueda por código
        # ============================================================
        lbl_foto = QLabel()
        ...
        # 1️⃣ PRIMERO: Intentar con url_foto (carpeta de imágenes local)
        url_foto = producto.get('url_foto')
        if url_foto:
            # Buscar en la carpeta imagenes_productos
            ruta = os.path.join('imagenes_productos', url_foto)
        ...
        # 2️⃣ SEGUNDO: Si no hay url_foto o falló, buscar por código
        if not pixmap or pixmap.isNull():
            codigo = producto.get('codigo_producto')
            if codigo:
                extensiones = ['.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp']
                for ext in extensiones:
                    ruta = os.path.join('imagenes_productos', f'{codigo}{ext}')
                    if os.path.exists(ruta):
                        pixmap = QPixmap(ruta)
                        if not pixmap.isNull():
                            lbl_foto.setPixmap(pixmap.scaled(140, 85, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
        ...
        # 3️⃣ TERCERO: Intentar con BLOB (si existe en la BD)
        if not pixmap or pixmap.isNull():
            foto_blob = producto.get('foto')
            if foto_blob and isinstance(foto_blob, bytes):
                pixmap = QPixmap()
                if pixmap.loadFromData(foto_blob):
                    lbl_foto.setPixmap(pixmap.scaled(140, 85, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
        
        # 4️⃣ Si nada funciona, mostrar placeholder
        if not pixmap or pixmap.isNull():
        ...
        warnings = []
        if not producto.get('foto') and not producto.get('url_foto'):
...
class VistaProductosUnificada(QDialog):
    def __init__(self, db: sqlite3.Connection, parent=None):
        super().__init__(parent)
        self.db = db
        self.controlador = ControladorProductos(db)
    ...
    def _crear_tab_productos(self):
        ...
        self.tabla.setHorizontalHeaderLabels(["ID", "Código", "Descripción", "Costo", "Venta", "Stock"])
        ...
        self.tabla_lotes.setHorizontalHeaderLabels(["ID", "N° Lote", "Fecha Venc.", "Inicial", "Actual"])
        ...
        self.tabla_lotes.setColumnWidth(2, 100)
        self.tabla_lotes.setColumnWidth(3, 80)
        ...
        self.btn_sincronizar.clicked.connect(self.sincronizar_productos)
    ...
    def gestionar_categorias(self):
        ...
        dialogo.setStyleSheet("background-color: #F0F2F5;")
        ...
        lista.setHorizontalHeaderLabels(["ID", "Nombre"])
        ...
        def eliminar_categoria():
            ...
            if fila >= 0:
                categoria_id = lista.item(fila, 0).text()
                self.categoria_modelo.eliminar(categoria_id)
                self._cargar_categorias()
                self._refrescar_lista_categorias(lista)
                self._cargar_categorias_catalogo()
    ...
    def _refrescar_lista_categorias(self, lista):
        ...
        for i, c in enumerate(categorias):
            lista.setItem(i, 0, QTableWidgetItem(c['id'][:8] + "..."))
            lista.setItem(i, 1, QTableWidgetItem(c['nombre']))
    ...
    def cargar_productos(self):
        ...
        if texto:
            cur.execute("""
                SELECT id, codigo_producto, descripcion, precio_costo, precio_venta, stock_actual 
                FROM productos WHERE activo=1 AND (codigo_producto LIKE ? OR descripcion LIKE ?) 
                LIMIT 100
            """, (f"%{texto}%", f"%{texto}%"))
        else:
            cur.execute("""
        ...
        for fila, prod in enumerate(registros):
            self.tabla.setItem(fila, 0, QTableWidgetItem(prod["id"][:8] + "..."))
            self.tabla.setItem(fila, 1, QTableWidgetItem(prod["codigo_producto"]))
            self.tabla.setItem(fila, 2, QTableWidgetItem(prod["descripcion"][:35]))
    ...
    def seleccionar_producto(self):
        ...
        fila = indices[0].row()
        producto_id = self.tabla.item(fila, 0).data(Qt.ItemDataRole.UserRole)
        self.producto_seleccionado_id = producto_id
        
        prod = self.controlador.obtener_producto(producto_id)
        if prod:
            ...
            # ✅ Cargar imagen desde url_foto o BLOB
            url_foto = prod.get('url_foto')
            if url_foto:
                ruta = os.path.join('imagenes_productos', url_foto)
                if os.path.exists(ruta):
                    pixmap = QPixmap(ruta)
            ...
            # Si no hay url_foto, intentar con BLOB
    ...
    def crear_lote(self):
        ...
        try:
            ...
            stock_ctrl = ControladorStock(self.db)
            lote_id = stock_ctrl.crear_lote(
            ...
            self.cargar_productos()
            self.cargar_stock()
            QMessageBox.information(self, "Éxito", f"✅ Lote creado correctamente.\nID: {lote_id[:8]}...")
        except Exception as e:
    ...
    def guardar_producto(self):
        try:
            ...
            QMessageBox.information(self, "Éxito", f"Producto creado (UUID: {producto_id[:8]}...)")
    ...
    def modificar_producto(self):
        ...
        try:
            self.controlador.modificar_producto(
    ...
    def eliminar_producto(self):
        ...
        confirm = QMessageBox.question(self, "Confirmar", "¿Eliminar producto seleccionado?",
                                       QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if confirm == QMessageBox.StandardButton.Yes:
            self.controlador.eliminar_producto(self.producto_seleccionado_id)
            self.cargar_productos()
            self.cargar_stock()
            self.cargar_catalogo()
            self.limpiar_formulario()
    ...
    def sincronizar_productos(self):
        try:
            ...
            self.cargar_productos()
            QMessageBox.information(self, "Sincronización", "✅ Productos sincronizados con Turso")
        except Exception as e:
    ...
    def _crear_tab_stock(self):
        ...
        self.tabla_stock.setHorizontalHeaderLabels(["ID", "Código", "Producto", "Stock", "Crítico", "Unidad", "Próx. Venc.", "Estado"])
        
        self.tabla_stock.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Interactive)
        self.tabla_stock.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Interactive)
        ...
        self.tabla_stock.horizontalHeader().setSectionResizeMode(7, QHeaderView.ResizeMode.Interactive)
    ...
    def sincronizar_stock(self):
        try:
            resultado = sincronizar_ahora(self.db)
            self.cargar_stock()
            QMessageBox.information(self, "Sincronización", "✅ Stock sincronizado con Turso")
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error en sincronización: {e}")
    ...
    def _crear_tab_mas_vendidos(self):
        ...
        self.tabla_mas_vendidos.setColumnCount(4)
        self.tabla_mas_vendidos.setHorizontalHeaderLabels(["ID", "Producto", "Cantidad Vendida", "Total Vendido"])
        self.tabla_mas_vendidos.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)
        self.tabla_mas_vendidos.setColumnWidth(0, 100)
        self.tabla_mas_vendidos.setColumnWidth(2, 100)
        self.tabla_mas_vendidos.setColumnWidth(3, 120)
    ...
    def cargar_mas_vendidos(self):
        cur = self.db.cursor()
        cur.execute("""
            SELECT p.id, p.descripcion, SUM(fd.cantidad) as total_cantidad, 
                   SUM(fd.cantidad * fd.precio_unitario) as total_venta
        ...
        if not top:
            ...
            self.tabla_mas_vendidos.setSpan(0, 0, 1, 4)
        ...
        for i, row in enumerate(top):
            self.tabla_mas_vendidos.setItem(i, 0, QTableWidgetItem(row['id'][:8] + "..."))
            self.tabla_mas_vendidos.setItem(i, 1, QTableWidgetItem(row['descripcion'][:40]))
            self.tabla_mas_vendidos.setItem(i, 2, QTableWidgetItem(f"{row['total_cantidad']:.0f}"))
            self.tabla_mas_vendidos.setItem(i, 3, QTableWidgetItem(f"${row['total_venta']:,.2f}"))
    ...
    def imprimir_mas_vendidos(self):
        ...
        for row in range(self.tabla_mas_vendidos.rowCount()):
            if self.tabla_mas_vendidos.item(row, 0).text() == "No hay ventas registradas":
                break
            producto = self.tabla_mas_vendidos.item(row, 1).text()
            cantidad = self.tabla_mas_vendidos.item(row, 2).text()
            total = self.tabla_mas_vendidos.item(row, 3).text()
            html += f"""
                <tr>
                    <td class="numero">{row + 1}</td>
                    <td>{producto}</td>
    ...
    def _crear_tab_ultimas_ventas(self):
        ...
        self.tabla_ultimas_ventas.setColumnCount(6)
        self.tabla_ultimas_ventas.setHorizontalHeaderLabels(["ID", "Fecha", "Factura", "Cliente", "Producto", "Cantidad"])
        self.tabla_ultimas_ventas.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.Interactive)
        self.tabla_ultimas_ventas.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeMode.Stretch)
        self.tabla_ultimas_ventas.setColumnWidth(0, 100)
        self.tabla_ultimas_ventas.setColumnWidth(1, 90)
        self.tabla_ultimas_ventas.setColumnWidth(2, 100)
        self.tabla_ultimas_ventas.setColumnWidth(3, 150)
        self.tabla_ultimas_ventas.setColumnWidth(5, 70)
    ...
    def cargar_ultimas_ventas(self):
        ...
        for fila, v in enumerate(ventas):
            self.tabla_ultimas_ventas.setItem(fila, 0, QTableWidgetItem(v['id'][:8] + "..."))
            self.tabla_ultimas_ventas.setItem(fila, 1, QTableWidgetItem(v['fecha']))
            self.tabla_ultimas_ventas.setItem(fila, 2, QTableWidgetItem(v['numero_factura']))
            self.tabla_ultimas_ventas.setItem(fila, 3, QTableWidgetItem(v['razon_social'][:25]))
            self.tabla_ultimas_ventas.setItem(fila, 4, QTableWidgetItem(v['descripcion'][:35]))
            self.tabla_ultimas_ventas.setItem(fila, 5, QTableWidgetItem(f"{v['cantidad']:.2f}"))
    ...
    def _crear_tab_catalogo(self):
        ...
        filtro_layout.addWidget(self.cmb_filtro_cat, 1)
        filtro_layout.addStretch()
        layout.addLayout(filtro_layout)

        # Scroll area para el catálogo
    ...
    def _cargar_categorias_catalogo(self):
        """Carga categorías en el filtro del catálogo"""
    ...
    def cargar_catalogo(self):
        """Carga el catálogo de productos con tarjetas"""
        ...
        if cat_id:
            cur.execute("""
                SELECT id, codigo_producto, descripcion, precio_venta, stock_actual, 
                       url_foto, precio_oferta
                FROM productos WHERE activo=1 AND categoria_id=? 
