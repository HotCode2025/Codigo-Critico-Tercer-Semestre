...
==================================================
Vista de Cheques con UUID
==================================================
📌 USO: Gestión de cheques
📌 CARACTERÍSTICAS:
    - CRUD completo con UUID
    - Estados: EN_CARTERA, DEPOSITADO, VENDIDO, ACREDITADO, RECHAZADO
    - Consulta BCRA (pendiente implementación completa)
...
class VistaCheques(QDialog):
    def __init__(self, db: sqlite3.Connection, parent=None):
        ...
        self.btn_sincronizar.clicked.connect(self.sincronizar_cheques)
    ...
    def _crear_tab_cheques(self, tipo):
        ...
        if tipo == "cartera":
            ...
            tabla.setHorizontalHeaderLabels(["ID", "N° Cheque", "Cliente", "Banco", "Emisión", "Vencimiento", "Importe", "Estado", "Acciones"])
        else:
            tabla.setColumnCount(10)
            tabla.setHorizontalHeaderLabels(["ID", "N° Cheque", "Cliente", "Banco", "Emisión", "Vencimiento", "Importe", "Estado", "Vendido a", "Acciones"])
    ...
    def cargar_clientes(self):
        """Carga clientes en el combo (con UUID)"""
    ...
    def _cargar_cheques_en_tabla(self, tab, tipo):
        """Carga cheques en la tabla correspondiente"""
        ...
        for i, ch in enumerate(cheques):
            tabla.setItem(i, 0, QTableWidgetItem(ch['id'][:8] + "..."))
            tabla.setItem(i, 1, QTableWidgetItem(ch['numero_cheque']))
            tabla.setItem(i, 2, QTableWidgetItem(ch.get('cliente_nombre', ch.get('cliente_id', ''))))
            tabla.setItem(i, 3, QTableWidgetItem(ch['banco']))
            tabla.setItem(i, 4, QTableWidgetItem(ch['fecha_emision']))
            tabla.setItem(i, 5, QTableWidgetItem(ch['fecha_vencimiento']))
            
            item_importe = QTableWidgetItem(f"${ch['importe']:,.2f}")
            item_importe.setTextAlignment(Qt.AlignmentFlag.AlignRight)
            tabla.setItem(i, 6, item_importe)
            ...
            elif ch['estado'] == 'RECHAZADO':
                item_estado.setForeground(QColor('#D32F2F'))
            tabla.setItem(i, 7, item_estado)
            
            if tipo == "vendidos":
                tabla.setItem(i, 8, QTableWidgetItem(ch.get('vendido_a', '')))
                ...
                tabla.setCellWidget(i, 9, btn_ver)
            else:
                ...
                tabla.setCellWidget(i, 8, btn_accion)
    ...
    def registrar_cheque(self):
        ...
        try:
            ...
            existente = self.cheque_modelo.obtener_por_numero(numero)
            if existente:
                raise ValueError(f"Ya existe un cheque con el número {numero}.")

            # Crear cobro temporal (sin cobro real)
            cobro_id = "TEMP-" + self.cheque_modelo.generar_uuid()
            
            cheque_id = self.ctrl_cheques.crear_cheque(
                cobro_id=cobro_id,
            ...
            self._mostrar_mensaje("Éxito", f"✅ Cheque registrado correctamente.\nID: {cheque_id[:8]}...")
    ...
    def acreditar_cheque(self, tab, tipo):
        ...
        numero_cheque = tabla.item(fila, 1).text()
    ...
    def rechazar_cheque(self, cheque_id, numero_cheque):
        """Marca un cheque como rechazado"""
        confirm = QMessageBox.question(self, "Confirmar", 
                                       f"¿Rechazar cheque N° {numero_cheque}?",
                                       QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        
        if confirm == QMessageBox.StandardButton.Yes:
            observaciones, ok = QInputDialog.getText(self, "Rechazar cheque", "Motivo del rechazo:")
            self.ctrl_cheques.rechazar(cheque_id, observaciones.strip() if ok and observaciones else None)
            self._mostrar_mensaje("Éxito", f"✅ Cheque {numero_cheque} marcado como rechazado.")
            self.cargar_cheques_cartera()
            self.cargar_cheques_vendidos()

    def ver_detalle_cheque(self, cheque):
        """Muestra el detalle de un cheque"""
        dialog = QDialog(self)
        ...
        datos.addWidget(QLabel(cheque['id'][:8] + "..."), 0, 1)
        datos.addWidget(QLabel("<b>N° Cheque:</b>"), 0, 2)
        datos.addWidget(QLabel(cheque['numero_cheque']), 0, 3)
        
        datos.addWidget(QLabel("<b>Cliente:</b>"), 1, 0)
        datos.addWidget(QLabel(cheque.get('cliente_nombre', '')), 1, 1)
        datos.addWidget(QLabel("<b>Banco:</b>"), 1, 2)
        datos.addWidget(QLabel(cheque['banco']), 1, 3)
        
        datos.addWidget(QLabel("<b>Importe:</b>"), 2, 0)
        datos.addWidget(QLabel(f"${cheque['importe']:,.2f}"), 2, 1)
        ...
        elif cheque['estado'] == 'RECHAZADO':
            lbl_estado.setStyleSheet("color: #D32F2F; font-weight: bold;")
        datos.addWidget(lbl_estado, 2, 3)
        
        datos.addWidget(QLabel("<b>F. Emisión:</b>"), 3, 0)
        datos.addWidget(QLabel(cheque['fecha_emision']), 3, 1)
        datos.addWidget(QLabel("<b>F. Vencimiento:</b>"), 3, 2)
        datos.addWidget(QLabel(cheque['fecha_vencimiento']), 3, 3)
        
        if cheque.get('vendido_a'):
            datos.addWidget(QLabel("<b>Vendido a:</b>"), 4, 0)
            datos.addWidget(QLabel(cheque['vendido_a']), 4, 1, 1, 3)
        
        if cheque.get('fecha_acreditacion'):
            datos.addWidget(QLabel("<b>Fecha Acreditación:</b>"), 5, 0)
            datos.addWidget(QLabel(cheque['fecha_acreditacion']), 5, 1, 1, 3)
        
        if cheque.get('factura_ids'):
            datos.addWidget(QLabel("<b>Facturas:</b>"), 6, 0)
            datos.addWidget(QLabel(cheque['factura_ids']), 6, 1, 1, 3)
    ...
    def mostrar_acciones_cheque(self, cheque):
        """Muestra acciones disponibles para un cheque"""
        dialog = QDialog(self)
        ...
        lbl_info = QLabel(
            f"Cliente: {cheque.get('cliente_nombre', '')}<br>"
            f"Importe: ${cheque['importe']:,.2f}<br>"
            f"Vencimiento: {cheque['fecha_vencimiento']}<br>"
            f"Estado: {cheque['estado']}"
        )
        ...
        btn_rechazar.setStyleSheet("background-color: #D32F2F; color: white; border-radius: 4px; padding: 8px 12px;")
        btn_rechazar.clicked.connect(lambda: self._rechazar_desde_dialogo(cheque['id'], cheque['numero_cheque'], dialog))
        ...
        btn_layout.addWidget(btn_rechazar)
        btn_layout.addStretch()
    ...
    def _rechazar_desde_dialogo(self, cheque_id, numero_cheque, dialog):
        observaciones, ok = QInputDialog.getText(self, "Rechazar cheque", "Motivo del rechazo:")
        if ok:
            self.ctrl_cheques.rechazar(cheque_id, observaciones.strip() if observaciones else None)
            self._mostrar_mensaje("Éxito", f"✅ Cheque {numero_cheque} rechazado.")
            dialog.accept()
            self.cargar_cheques_cartera()
            self.cargar_cheques_vendidos()

    def consultar_bcra(self, tab):
        """Consulta el BCRA para un cheque seleccionado"""
        tabla = tab.tabla
        fila = tabla.currentRow()
        if fila < 0:
        ...
        numero_cheque = tabla.item(fila, 1).text()
        banco = tabla.item(fila, 3).text()
        
        # Buscar código de entidad (simplificado)
        codigos_entidad = {
            'BANCO NACIÓN': '011',
            'BANCO PROVINCIA': '014',
            'BANCO CIUDAD': '020',
        ...
        codigo = None
        for key, value in codigos_entidad.items():
            if key in banco.upper():
                codigo = value
                break
        
        if not codigo:
            self._mostrar_mensaje("BCRA", 
                                 f"No se pudo identificar el código de entidad para '{banco}'\n"
                                 f"Por favor, ingrese el código manualmente.",
                                 QMessageBox.Icon.Warning)
            codigo, ok = QInputDialog.getText(self, "Código de entidad", 
                                             f"Ingrese el código de entidad para {banco}:")
            if not ok or not codigo:
                return
        
        try:
            resultado = self.ctrl_cheques.consultar_cheque_bcra(int(codigo), numero_cheque)
            
            if resultado:
                self._mostrar_mensaje("BCRA - Cheque Denunciado", 
                                     f"⚠️ El cheque N° {numero_cheque} fue denunciado.\n\n"
                                     f"Datos de la denuncia:\n{resultado}",
                                     QMessageBox.Icon.Warning)
            else:
                self._mostrar_mensaje("BCRA - Cheque Válido", 
                                     f"✅ El cheque N° {numero_cheque} NO fue denunciado.",
                                     QMessageBox.Icon.Information)
                
        except Exception as e:
            self._mostrar_mensaje("Error", f"Error consultando BCRA: {e}", QMessageBox.Icon.Critical)

    def sincronizar_cheques(self):
        """Sincroniza cheques con Turso"""
        try:
            resultado = sincronizar_ahora(self.db)
            self.cargar_cheques_cartera()
            self.cargar_cheques_vendidos()
            self._mostrar_mensaje("Sincronización", "✅ Cheques sincronizados con Turso")
        except Exception as e:
