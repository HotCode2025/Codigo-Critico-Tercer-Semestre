...
==================================================
Vista de Parámetros con UUID
==================================================
📌 USO: Configuración general del sistema
📌 CARACTERÍSTICAS:
    - Datos de la empresa
...
class VistaParametros(QDialog):
    def __init__(self, db: sqlite3.Connection, parent=None):
        ...
        self.txt_escala = LineEditBlanco()
        self.txt_escala.setText("1.0")
        ...
        form_facturacion.addWidget(LabelCampoAzul("Escala Visual:"), 5, 0)
        form_facturacion.addWidget(self.txt_escala, 5, 1)
        ...
        btn_sincronizar.setMinimumHeight(38)
        btn_sincronizar.setStyleSheet("background-color: #4CAF50; color: white;")
        btn_sincronizar.clicked.connect(self.sincronizar_parametros)
    ...
    def cargar_datos(self):
        ...
        if row:
            ...
            self.txt_escala.setText(str(row['escala_visual'] or 1.0))
    ...
    def guardar_parametros(self):
        try:
            ...
            """, (
                ...
                float(self.txt_escala.text() or 1.0)
            ))
            self.db.commit()
            
            # Actualizar variable de entorno de escala
            import os
            os.environ["QT_SCALE_FACTOR"] = str(self.txt_escala.text() or 1.0)
            
            self._mostrar_mensaje("Éxito", "Parámetros guardados correctamente.\n\n"
                                         "La escala visual se aplicará al reiniciar la aplicación.")
            self.accept()
            
        except Exception as e:
            self._mostrar_mensaje("Error", f"No se pudo guardar: {e}", QMessageBox.Icon.Critical)

    def sincronizar_parametros(self):
        """Sincroniza parámetros con Turso"""
        try:
            resultado = sincronizar_ahora(self.db)
            self.cargar_datos()
            self._mostrar_mensaje("Sincronización", "✅ Parámetros sincronizados con Turso")
        except Exception as e:
