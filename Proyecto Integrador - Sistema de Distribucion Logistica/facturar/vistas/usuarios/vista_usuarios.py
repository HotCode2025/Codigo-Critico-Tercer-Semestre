...
==================================================
Vista de Usuarios con UUID
==================================================
📌 USO: Gestión de usuarios del sistema
📌 CARACTERÍSTICAS:
    - CRUD completo con UUID
    - Roles: admin, preventista, cliente
...
class VistaUsuarios(QDialog):
    def __init__(self, db: sqlite3.Connection, parent=None):
        ...
        self.tabla.setHorizontalHeaderLabels(["ID", "Usuario", "Rol", "Asociado a", "Estado", "Creado", "Acciones"])
        ...
        self.btn_sincronizar.setStyleSheet(btn_style_verde)
        ...
        self.btn_sincronizar.clicked.connect(self.sincronizar_usuarios)
    ...
    def cargar_usuarios(self):
        ...
        for i, u in enumerate(usuarios):
            # ID
            self.tabla.setItem(i, 0, QTableWidgetItem(u['id'][:8] + "..."))
            ...
            # Creado
            self.tabla.setItem(i, 5, QTableWidgetItem(u.get('created_at', '')[:10] if u.get('created_at') else ''))
            ...
            self.tabla.setCellWidget(i, 6, btn_ver)
    ...
    def ver_detalle_usuario(self, usuario):
        ...
        datos.addWidget(QLabel(usuario['id'][:8] + "..."), 0, 1)
    ...
    def sincronizar_usuarios(self):
        """Sincroniza usuarios con Turso"""
        try:
            resultado = sincronizar_ahora(self.db)
            self.cargar_usuarios()
            QMessageBox.information(self, "Sincronización", "✅ Usuarios sincronizados con Turso")
        except Exception as e:
