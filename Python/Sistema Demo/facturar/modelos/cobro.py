...
Modelo de Cobro: registra entregas de dinero y actualiza la cuenta corriente.
"""
import sqlite3
...
class Cobro:
    ...
    def registrar(self, cliente_id: int, importe: float,
        ...
        # Insertar cobro
        cur = self.db.cursor()
        cur.execute("""INSERT INTO cobros (cliente_id, fecha, importe, medio_pago, observaciones)
                       VALUES (?, ?, ?, ?, ?)""",
                    (cliente_id, date.today().isoformat(), importe, medio_pago, observaciones))
        self.db.commit()
        cobro_id = cur.lastrowid

        # Actualizar saldo cuenta corriente y registrar movimiento
        ...
        if row:
            ...
            cur.execute("""INSERT INTO cuenta_corriente_movimientos
                           (cliente_id, fecha, tipo_movimiento, referencia_id, importe,
                            saldo_resultante)
                           VALUES (?, ?, 'COBRO', ?, ?, ?)""",
                        (cliente_id, date.today().isoformat(), cobro_id, importe, nuevo_saldo))
            self.db.commit()
        return cobro_id

    def obtener_por_id(self, cobro_id: int) -> Optional[Dict[str, Any]]:
        cur = self.db.cursor()
