package test;

import enumeraciones.Dias;

import enumeraciones.Continentes;

public class TestEnumeracion {
    public static void main(String[] args) {
      //  System.out.println("Dia 1: "+Dias.LUNES);
       // indicarDiaSemana(Dias.LUNES); //Las enumeraciones se tratan como cadenas
        //Ahora no se deben utilizar comillas, se accede a traves de un operador de punto
        
            System.out.println("Continente Nro. 4: "+Continentes.AMERICA);
            System.out.println("Nro de paises en el 4to continente: "
                    +Continentes.AMERICA.getPaises());
            System.out.println("Nro de habitantes en el 4to continente: "
                    +Continentes.AMERICA.getHabitantes());
            
            //Recorrer todos los continentes
            System.out.println("\n--- Lista de Continentes ---");
        for (Continentes c : Continentes.values()) {
            System.out.println("Continente: " + c 
                    + " | Paises: " + c.getPaises() 
                    + " | Habitantes: " + c.getHabitantes());
            }
    }
   
    private static void indicarDiaSemana(Dias dias){
        switch(dias){
            case LUNES:
                System.out.println("Primer dia de la semana");
                break;
            case MARTES:
                System.out.println("Segundo dia de la semana");
                break;
            case MIERCOLES:
                System.out.println("Tercer dia de la semana");
                break;
            case JUEVES:
                System.out.println("Cuarto dia de la semana");
                break;
            case VIERNES:
                System.out.println("Quinto dia de la semana");
                break;
            case SABADO:
                System.out.println("Sexto dia de la semana");
                break;
            case DOMINGO:
                System.out.println("Septimo dia de la semana");
                break;
            default:
                System.out.println("Día no válido");
                break;
            //Agregar todos los dias de la semana
            //Agregar default
        }
    }
}
